...
def judge(root, title, stage, cur_candidate, judges, pause, remain, is_final):
	def dependencies(caller):
		caller.root = root

		caller.statusbar = tools.status_bar(caller)
		if is_final:
			caller.statusbar.mode_judge_final()

		caller.varJudges = [tk.StringVar(caller) for judge in judges]
		...
		caller.info = InfoBox(title, caller)
		caller.statusbox = StatusBox(caller)

		caller.btnPause = ttk.Button(caller, text = 'Pause', command = lambda: pause(btnPause))

	view = Judge(dependencies, root)
